...
var _ = Describe("Config", func() {
	...
	Context("with a deployed app", func() {
		...
		It("can set and list environment variables", func() {
			sess, err := Run("deis config:set POWERED_BY=midi-chlorians")
			...
			sess, err = Run(`curl -sL "%s"; echo`, testApp.URL)
			Eventually(sess).Should(Say("Powered by midi-chlorians"))
			Eventually(sess).Should(Exit(0))

			sess, err = Run("deis run env -a %s", testApp.Name)
			Expect(err).NotTo(HaveOccurred())
		...
		It("can set an integer environment variable", func() {
			sess, err := Run("deis config:set FOO=1 -a %s", testApp.Name)
			Expect(err).NotTo(HaveOccurred())
			Eventually(sess).Should(Say("Creating config"))
			Eventually(sess, DefaultMaxTimeout).Should(Say("=== %s Config", testApp.Name))
		...
		It("can set multiple environment variables at once", func() {
			sess, err := Run("deis config:set FOO=null BAR=nil -a %s", testApp.Name)
			Expect(err).NotTo(HaveOccurred())
			Eventually(sess).Should(Say("Creating config"))
			Eventually(sess, DefaultMaxTimeout).Should(Say("=== %s Config", testApp.Name))
			Eventually(sess).Should(Exit(0))
		...
		It("can set an environment variable containing spaces", func() {
			sess, err := Run(`deis config:set -a %s POWERED_BY=the\ Deis\ team`, testApp.Name)
			Expect(err).NotTo(HaveOccurred())
			Eventually(sess).Should(Say("Creating config"))
			Eventually(sess, DefaultMaxTimeout).Should(Say("=== %s Config", testApp.Name))
			...
			sess, err = Run("deis run -a %s env", testApp.Name)
			Expect(err).NotTo(HaveOccurred())
...
multiline string.`
			...
			sess, err := Run(`deis config:set -a %s FOO='%s'`, testApp.Name, value)
			Expect(err).NotTo(HaveOccurred())
			Eventually(sess).Should(Say("Creating config"))
			Eventually(sess, DefaultMaxTimeout).Should(Say("=== %s Config", testApp.Name))
		...
		It("can set an environment variable with non-ASCII and multibyte chars", func() {
			sess, err := Run("deis config:set FOO=讲台 BAR=Þorbjörnsson BAZ=ноль -a %s",
				testApp.Name)
			Expect(err).NotTo(HaveOccurred())
			Eventually(sess).Should(Say("Creating config"))
			Eventually(sess, DefaultMaxTimeout).Should(Say("=== %s Config", testApp.Name))
		...
		It("can unset an environment variable", func() {
			sess, err := Run("deis config:set -a %s FOO=xyzzy", testApp.Name)
			Expect(err).NotTo(HaveOccurred())
			Eventually(sess).Should(Say("Creating config"))
			Eventually(sess, DefaultMaxTimeout).Should(Say("=== %s Config", testApp.Name))
			...
			sess, err = Run("deis config:unset -a %s FOO", testApp.Name)
			Expect(err).NotTo(HaveOccurred())
		...
		It("can pull the configuration to an .env file", func() {
			sess, err := Run("deis config:set -a %s BAZ=Freck", testApp.Name)
			Expect(err).NotTo(HaveOccurred())
			Eventually(sess).Should(Say("Creating config"))
			Eventually(sess, DefaultMaxTimeout).Should(Say("=== %s Config", testApp.Name))
			...
			sess, err = Run("deis config:pull -a %s", testApp.Name)
			Expect(err).NotTo(HaveOccurred())
...
FOO=bar`)
			...
			sess, err := Run("deis config:push -a %s", testApp.Name)
			Expect(err).NotTo(HaveOccurred())
			Eventually(sess, DefaultMaxTimeout).Should(Exit(0))
