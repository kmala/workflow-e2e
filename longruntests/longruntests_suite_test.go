...
var (
	...
	exampleRepos      = []string{"example-go", "example-php", "example-python-django", "example-dockerfile-http"}
)
...
var _ = BeforeEach(func() {
	...
	os.Chdir(testRoot)
	for _, repo := range exampleRepos {
		output, err = Execute(`git clone https://github.com/deis/` + repo + `.git`)
		Expect(err).NotTo(HaveOccurred(), output)
	}
